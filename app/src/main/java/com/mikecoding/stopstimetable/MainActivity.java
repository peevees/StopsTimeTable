...
public class MainActivity extends AppCompatActivity implements ApiInterface {
    ...
    protected void onCreate(Bundle savedInstanceState)  {
        ...
        if (sharedPref.contains("history")) {
            historyArr.add(sharedPref.getString("history", ""));
        }
        ...
        NoFilterArrayAdapter historyAdapter = new NoFilterArrayAdapter(this, R.layout.history_item, R.id.autocomplete_item, historyArr);
        inputText.setThreshold(1);

        if (!historyArr.isEmpty()) {
            inputText.setAdapter(historyAdapter);
        }
    ...
    public void searchStations(View view){
        if (inputText.getText().toString().isEmpty()) {
            ...
            editor.putString("history", inputText.getText().toString());
            editor.apply();
